...
"""Sim2Sim: Isaac Lab → MuJoCo deployment for HimLoco Go2 policy."""
...
class HimLocoSim2Sim:
    def __init__(self, xml_path, export_dir):
        ...
        print(f"[INFO] Sim2Sim initialized.")
    ...
    def _load_metadata(self, model):
        ...
        # qpos and qvel have different addresses (free joint: 7 qpos, 6 qvel).
        ...
        for i in range(self.m.nu):
            jnt_id = self.m.actuator_trnid[i, 0]  # first transmission target
        ...
        assert len(self.qpos_joint_order) == self.num_action, \
            f"Expected {self.num_action} actuated joints, got {len(self.qpos_joint_order)}"
        ...
        for lab_joint_name in self.lab_order:
            act_name = lab_joint_name.replace('_joint', '')
            act_id = mujoco.mj_name2id(self.m, mujoco.mjtObj.mjOBJ_ACTUATOR, act_name)
    ...
    def _get_joint_positions(self):
        ...
        pos = np.zeros(self.num_action, dtype=np.float32)
    ...
    def _get_joint_velocities(self):
        ...
        vel = np.zeros(self.num_action, dtype=np.float32)
    ...
    def run(self):
        ...
        print("[INFO] Starting sim2sim loop...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="HimLoco Sim2Sim")
